...
my($NAME) = $0 =~ /([\w\.]+)$/;
my $VERSION = ('$Snapshot: /Convert-Binary-C/0.58_01 $' =~ /([^\/\s]+)\s*\$$/)[0];
my $MESSAGE = "\nThis is $NAME, v$VERSION ($0).\n";
my %OPT;

unless( GetOptions( \%OPT, qw(
          cc|c=s ppout|p=s temp|t=s binfile=s datafile=s cfgfile=s
...
$OPT{binfile}  ||= "$OPT{cc}.bin";
$OPT{datafile} ||= "$OPT{cc}.dat";
$OPT{cfgfile}  ||= "$OPT{cc}.dat";
...
my $data = pack 'C*', map { 1 + rand 255 } 1 .. $c->sizeof('test');
my $test = $c->unpack('test', $data);
my $init = $c->initializer('test', $test);
...
my $hex = hexdump($bin);
IO::File->new(">$OPT{datafile}")->print(Data::Dumper->new([$test], ['data'])->Indent(1)->Dump);
IO::File->new(">$OPT{binfile}")->print($hex);

if (length $bin == $c->sizeof('test')) {
  print STDERR "YES!!!!\n";
}

my $xxx  = $c->pack('test', $test);
my $pass = 0;
my $fail = 0;

for my $i (0 .. $c->sizeof('test')-1) {
  my $a = ord substr $xxx, $i, 1;
...
printf STDERR "size=%d pass=$pass fail=$fail\n", $c->sizeof('test');
...
sub _preset_names
{
  qw(
    ...
    arch_v3 arch_v8 ARCHITECTURE ardent arm arm2 arm32 arm6 ARM_ARCH_2
    ARM_ARCH_3 ARM_ARCH_3M ARM_ARCH_4 ARM_ARCH_4T ARM_ARCH_5 ARM_ARCH_5E
    ARM_ARCH_5T ARM_ARCH_5TE arm_elf arm_oabi ARMEB ARMEL ARMWEL atarist
    athlon athlon_sse att386 att3b AUX AUX_SOURCE AVR AVR_ARCH AVR_ASM_ONLY
    ...
    PROTOTYPES ps2 psos PTHREAD_USE_D4 PTHREADS PTRDIFF_TYPE PWB pyr QNX
    R3000 R4000 REENTRANT REGPARM RELOCATABLE RES REVARGV Rhapsody rios
    RISC6000 riscbsd riscix riscos ROSE rs6000 RT rtasim rtems s390 s390x
    s64_t SA110 SCO SCO_C_DIALECT SCO_COFF SCO_DS SCO_ELF SCO_ODS_30
    SCO_XPG_VERS scs semi sequent SEQUENT_ sgi SGI_SOURCE sh sh1 sh2 sh3
    SH3E SH4 SH4_NOFPU SH4_SINGLE SH4_SINGLE_ONLY SH5 SHARED SHMEDIA
    SHRT_MAX simulator sinix SIZE_INT SIZE_LONG SIZE_PTR SIZE_TYPE SNI
    SOCKET_SOURCE SOCKETS_SOURCE SOFT_FLOAT SOFTFP SOLARIS_THREADS sony
    sony_news sonyrisc sparc sparc64 sparc_v8 sparc_v9 sparclet sparclite
    sparclite86x sparcv9 spectrum spur SSE SSE2 stardent STATIC
    STD_INCLUDE_DIR STDC STDC_EXT stdcall STDCPP stratos STRICT_ANSI
    STRICT_BSD STRICT_NAMES sun sun3 sun386 Sun386i supersparc SVID SVR3
    SVR4 SVR4_2 SVR4_ABI SVR4_SOURCE svr5 SX system SYSTYPE_BSD
    SYSTYPE_BSD43 SYSTYPE_BSD44 SYSTYPE_SVR4 SYSTYPE_SVR5 SYSTYPE_SYSV SYSV
    SYSV3 SYSV4 SYSV5 sysV68 sysV88 SYSV_SOURCE tahoe Tek4132 Tek4300
    THREAD_SAFE thumb THUMB_INTERWORK THUMBEB THUMBEL titan TM3200 TM5400
    TM5600 TMS320C30 TMS320C31 TMS320C32 TMS320C33 TMS320C3x TMS320C40
    TMS320C44 TMS320C4x tower tower32 tower32_200 tower32_600 tower32_700
    tower32_800 tower32_850 TOWER_ASM tss tune_ tune_athlon tune_athlon_sse
    tune_i386 tune_i486 tune_i586 tune_i686 tune_k6 tune_k6_2 tune_k6_3
    tune_pentium tune_pentium2 tune_pentium3 tune_pentium4 tune_pentium_mmx
    tune_pentiumpro tune_v10 tune_v3 tune_v8 u370 u3b u3b2 u3b20 u3b200
    u3b20d u3b5 u64_t uclinux UINT128_T ultrix UMAXV unaligned UnicomPBB
    UnicomPBD UNICOS UNICOSMK unix UNIX95 UNIX99 unixpc unos USE_BSD
    USE_FILE_OFFSET64 USE_GNU USE_INIT_FINI USE_ISOC9X USE_LARGEFILE
    USE_LARGEFILE64 USE_MISC USE_POSIX USE_POSIX199309 USE_POSIX199506
    USE_POSIX2 USE_REENTRANT USE_RTC USE_SVID USE_UNIX98 USE_XOPEN
    USE_XOPEN_EXTENDED USES_INITFINI USG USGr4 USGr4_2 UTek Utek UTS UWIN
    uxpm uxps v20 v33 v850 v850e v850e1 v850ea v851 vax venix VMESA VMS
    VSTA vxworks vxworks_5 wchar_t WCHAR_TYPE we32000 WIN32 WINNT WINT_TYPE
    WRS_R3K_EXC_SUPPORT X86_ x86_64 X_FLOAT xenix Xenix286 XOPEN_C
    XOPEN_SOURCE XOPEN_SOURCE_EXTENDED XPG2 XPG2_EXTENDED XPG3
    XPG3_EXTENDED XPG4 XPG4_EXTENDED XSCALE xstormy16 XTENSA XTENSA_EB
...
ATTENTION: The floating point format used by the currently tested
           compiler appears to be IEEE, but Convert::Binary::C was
           built without the 'ieeefp' feature. Don't try to handle
...
ATTENTION: The currently tested compiler does not seem to use IEEE
           floating point format. Instead, the format I found was
           compatible with the floating point format used natively
           by your machine. For some reason, Convert::Binary::C
           was built with the 'ieeefp' feature, which is not very
           clever if my tests were correct. However, don't try to
           handle any floating point values in this configuration.
ENDMSG
      }
      else {
        $self->_msg( <<ENDMSG );
...
ATTENTION: The currently tested compiler does not seem to use IEEE
           floating point format, nor does it seem to use a format
           compatible with the floating point format used natively
           by your machine. Don't try to handle any floating point
           values with Convert::Binary::C for this configuration.
ENDMSG
      }
    }
  }
...
TEMP
  ...
  my $user_ppout = exists $self->{ppout};
...
TEMP
      ...
      if( $self->{can_build} ) {
        $self->_msg("I can also build executable files.");
        $res = $self->_run_temp;
